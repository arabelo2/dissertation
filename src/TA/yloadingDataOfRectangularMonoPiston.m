...
%             end
            ...
            % Peak amplitude
            Pp(yy, zz) = max(abs(p_temp));
            
            % Peak-to-peak amplitude
            Ppp(yy, zz) = max(p_temp) - min(p_temp); 
            
            % Root mean square amplitude
            Prms(yy, zz) = (max(abs(p_temp)) - min(abs(p_temp)))/(2*sqrt(2));            
        end
    end
end
...
figure(20)
imagesc(z(z_idx)*1e3, y(y_idx)*1e3, abs(Ppp(y_idx, z_idx))/max(max(abs(Ppp))))
% figure(200)
...
ylabel('y(mm)', 'Color', 'k', 'interpreter', 'latex')
title('Pressão Normalizada - Excitação Tipo I e plano x = 0', 'Color', 'k', 'interpreter', 'latex')
grid on
grid minor
...
figure(8)
mesh(z(z_idx)*1e3, y(y_idx)*1e3, abs(Ppp(y_idx, z_idx))/max(max(abs(Ppp))))
shading interp
colormap(jet)
...
title('Pressão Normalizada - Excitação Tipo I e plano x = 0', 'FontWeight', 'bold', 'Color', 'k', 'interpreter', 'latex')
grid on
grid minor
set(gca,'Ydir','reverse');
set(gca, 'FontName', 'Times New Roman', 'FontSize', 20)
