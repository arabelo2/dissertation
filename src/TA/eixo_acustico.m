...
xmin = 0; % xmin = -(2*a+kerf)*(M/2+1);
xmax = 0; % xmax = (2*a+kerf)*(M/2+1);
ymin = 0;
ymax = +0;
zmin = 0.002;
...
figure(6)
hold on
plot(z, cellfun(@max, P(:))/max(cellfun(@max, P(:))))
...
ylabel('Pressão Normalizada', 'FontSize', 20, 'FontWeight', 'bold', 'Color', 'k', 'interpreter', 'latex')
ylim([0, 1.2])
...
title('Formas de onda de pressão no eixo - Excitações tipos I e II com plano x = 0',  'FontSize', 20, 'FontWeight', 'bold', 'Color', 'k', 'interpreter', 'latex')
