...
py=120;
s2=load(['E:\FileHistory\arabelo@hpe.com\RABELOAL11\Data\C\Temp\PPGEM\Dissertação\Programa\Matuda\0graus-2015-03-01\0graus\DADOS0H200.DAT']); % ou 78, ou 76, veja se tem muita diferença
s2_temp=reshape(s2,N,py);
vn = resample(s2_temp(:, 60), 320, 100);
% vn = vn(find(vn == max(vn))-65:find(vn == max(vn)) + 215)/max(abs(vn)); %100MHz
% vn = vn(find(vn == max(vn))-210:find(vn == max(vn)) + 320)/max(abs(vn)); %320MHz
vn = vn(find(vn == max(vn)):find(vn == max(vn)) + 2*1024)/max(abs(vn)); %320MHz
save sinalvn0h200 vn
figure(1)
subplot(2,1,1)
plot(vn)
grid on
